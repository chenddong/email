...
public class SendEmailUtils {
	      ...
	      public void send(){
	          ...
	          try {
	              mimeMessage.setFrom(new InternetAddress(from,"service@gangguanhui.com")); 
	              mimeMessage.addRecipient(Message.RecipientType.TO, new InternetAddress(to));
	              mimeMessage.setSubject("我的账户密码重置");
	              mimeMessage.setSentDate(new Date());
	              mimeMessage.setText("您在PC端使用了密码重置功能，请点击下面链接重置密码:\n"
	                      + "http://localhost:8080/rg/ResetPassword?id="
	                     + id);
      ...
      public static void main(String[] args) {
	          SendEmailUtils s = new SendEmailUtils("1354685419@qq.com",
	                  "20160611121859OYI7W");
		      s.send();
		      System.out.println("s.id:"+s.id.toString()+"^^打印信息^^"+"s:"+s.to);
	  }
}
